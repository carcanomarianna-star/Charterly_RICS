...
# 5. Pathway Logic
html = re.sub(r'function rPathwayGuidance\(C\) \{.*?\}\nfunction pgInit\(\) \{.*?\}\s*function loadScript\(src, cb\) \{', 'function loadScript(src, cb) {', html, flags=re.DOTALL)
...
html = html.replace("if (S.body === \"CICES\") { rCPD_CICES(C); return; }", "")
